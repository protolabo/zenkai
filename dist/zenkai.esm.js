import { isIterable as isIterable$1, hasOwn as hasOwn$1, pascalCase as pascalCase$1, isCollection as isCollection$1, isEmpty as isEmpty$1, isString as isString$1, all as all$1, isNullOrUndefined as isNullOrUndefined$1, isObject as isObject$1, isNullOrWhitespace as isNullOrWhitespace$1, valOrDefault as valOrDefault$1, isFunction as isFunction$2 } from '@std/index.js';
...
const isElementNode = (obj) => !isNullOrUndefined$1(obj) && obj.nodeType === Node.ELEMENT_NODE;

/* istanbul ignore next */
const isDocumentFragmentNode = (obj) => !isNullOrUndefined$1(obj) && obj.nodeType === Node.DOCUMENT_FRAGMENT_NODE;
...
function isHTMLElementKind(element, kinds) {
    ...
    return kinds.some((kind) => {
        ...
        var interfaceName = `HTML${hasOwn$1(TagNameMapping, name) ? TagNameMapping[name] : pascalCase$1(name)}Element`;

        if (!(isInstanceOf(window[interfaceName]) || hasTag(name))) {
            return false;
        }

        if (isCollection$1(type) && !isEmpty$1(type)) {
...
function isInElement(element, target) {
    if (!all$1([element, target], isHTMLElement)) {
        throw new TypeError("Bad argument: The given 'element' is not a valid HTML Element");
    }
    ...
    return all$1([[top2, top1], [left2, left1], [right1, right2], [bottom1, bottom2]], (inner, outer) => inner <= outer);
}
...
function addClass(element, value) {
    ...
    if (Array.isArray(value)) {
        ...
        if (isNullOrWhitespace$1(element.className)) {
...
function addAttributes(element, attribute, validAttributes) {
    ...
    if (!isObject$1(attribute)) {
        return element;
    }

    const isValid = (key) => GLOBAL_ATTRIBUTES.includes(key) || isNullOrUndefined$1(validAttributes) || validAttributes.includes(key);
...
function changeSelectedValue(select, optValue) {
    ...
    if (!(isString$1(optValue) || isObject$1(optValue))) {
    ...
    const eqHandler = isString$1(optValue) ? stringEq : objectEq;
...
function createEmptyElement(tagName, _validAttributes, _attributes) {
    ...
    if (isObject$1(_attributes)) {
        addAttributes(element, _attributes, valOrDefault$1(_validAttributes, ""));
    }
...
function createElement(tagName, _validAttributes, _attributes, _content) {
    ...
    if (!isNullOrUndefined$1(_content)) {
...
function createDocFragment(_children) {
    ...
    if (!isNullOrUndefined$1(_children)) {
...
function addContent(element, content) {
    if (!(isNode(content) || isIterable$1(content))) {
        return element;
    }

    if (isNode(content) || isString$1(content)) {
...
function getElement(selector, _container) {
    ...
    if (isNullOrWhitespace$1(selector)) {
        return null;
    }
...
function cloneTemplate(template, deep) {
    return template ? document.importNode(template.content, valOrDefault$1(deep, true)) : template;
}
...
function findAncestorIter(target, pred, max) {
    if (isNullOrUndefined$1(target) || max === 0) {
        return null;
    }
...
function getUrlParams(prop, defValue) {
    ...
    if (isNullOrWhitespace$1(search)) {
        return null;
    }
    ...
    if (prop) {
        return valOrDefault$1(params[prop], defValue);
    }
...
function queryBuilder(query, ignoreNullOrEmpty = false) {
    ...
    Object.keys(query).forEach((prop) => {
        if (!ignoreNullOrEmpty || !isNullOrWhitespace$1(query[prop])) {
